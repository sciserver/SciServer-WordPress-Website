...
<?php
	...
	function check_remote_user() {
		...
		if (! $username) {
			// We should also check for the presence of a session variable.
			if (session_id() === "") { session_start(); }
			$username = $_SESSION['SM_JHEDID'];
		}
		...
		if (! $username) {
			...
			if ($this->allow_wp_auth()) {
				...
				// We should try to redirect to the login page, in order to correct this situation.
 				global $redirect_to;
                		$login_uri = $this->_generate_uri($this->options['login_uri'], wp_login_url($redirect_to));
		...
		if (! $user)  {
			...
			// If auto_create_user is set, but the user does not appear to be a member of this blog yet ... then we should try to add the access!
			if (((bool) $this->options['auto_create_user']) && (! is_super_admin($user->ID)) && (! is_user_member_of_blog($user->ID, get_current_blog_id()) ) )
			{
